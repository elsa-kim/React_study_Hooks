...
useMemo : 함수형 컴포넌트 내부에서 발생하는 연산 최적화, 렌더링하는 과정에서 특정값 바뀌었을 때만 연산 실행하고 원하는 값 바뀌지 않았다면 이전 연산 결과 다시 사용
  - 숫자, 문자열, 객체처럼 일반 값 재사용할 때 사용

useCallback : 첫번째 파라미터에는 생성하고 싶은 함수, 두번째 파라미터에는 배열(어떤값 바뀌었을때 함수 새로 생성해야 하는지 명시) 넣음, 함수 내부에서 상태값에 의존해야 할 때 그 값을 반드시 두번째 파라미터에 포함
  - 함수 재사용할 때 사용
  
useRef : 특정 DOM 직접 선택할 때 사용, useRef()로 Ref 객체 만들고 선택하고 싶은 DOM에 ref값으로 설정 => Ref객체의 .current 값은 원하는 DOM 가리킴
  - 컴포넌트 로컬 변수(렌더링과 상관없이 바뀔 수 있는 값) 사용할때도 활용, ref 안 값 바뀌어도 컴포넌트 렌더링 되지 않으므로 렌더링과 관련되지 않은 값을 관리할 때만 사용
